// This file is as direct a copy as possible of the internal tree view widget
...
export function setTreeSelectorTheme(injectedTheme: any): void {
    theme = injectedTheme;
}
...
class TreeList {
    ...
    applyFilter() {
        ...
        this.filteredNodes = this.flatNodes.filter((flatNode) => {
            ...
            if (searchTokens.length > 0) {
                const nodeText = this.getSearchableText(
                    flatNode.node,
                ).toLowerCase();
        ...
        if (this.lastSelectedId) {
            this.selectedIndex = this.findNearestVisibleIndex(
                this.lastSelectedId,
            );
    ...
    recalculateVisualStructure() {
        ...
        const visibleIds = new Set(
            this.filteredNodes.map((n) => n.node.entry.id),
        );
        ...
        const findVisibleAncestor = (nodeId) => {
            ...
            while (currentId !== null) {
                ...
                currentId =
                    entryMap.get(currentId)?.node.entry.parentId ?? null;
            }
            return null;
        };
    ...
    updateNodeLabel(entryId, label, labelTimestamp) {
        for (const flatNode of this.flatNodes) {
            if (flatNode.node.entry.id === entryId) {
                ...
                flatNode.node.labelTimestamp = label
    ...
    render(width) {
        ...
        if (this.filteredNodes.length === 0) {
            ...
            lines.push(
                truncateToWidth(
                    theme.fg("muted", `  (0/0)${this.getStatusLabels()}`),
                    width,
                ),
            );
            return lines;
    ...
    handleInput(keyData) {
        ...
        if (kb.matches(keyData, "tui.select.up")) {
            ...
            this.filterMode =
                this.filterMode === "no-tools" ? "default" : "no-tools";
            this.foldedNodes.clear();
            this.applyFilter();
